...
- File via REST `/rest/api/3/issue`: `summary` = line 1 of `.claude/tasks/artifacts/<slug>/issue.md` with its `# ` stripped, `description` = the rest, both read from the file into the JSON payload (`jq -Rs`), never retyped.
