...
- Search Jira (REST `/rest/api/3/search`) for an open issue with the same goal. Reuse + reference its key; never open a second. Else draft, get approval, create via REST `/rest/api/3/issue`.
