...
- Search `gh issue list --state all` for the same goal; `spec-author` reads any candidate with `bash .claude/scripts/issue-view.sh <number>`, never raw `gh issue view`. Reuse + reference its number; never open a second.
