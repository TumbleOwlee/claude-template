...
- File via the MCP create-issue tool: `summary` = line 1 of `.claude/tasks/artifacts/<slug>/issue.md` with its `# ` stripped (`sed -n '1s/^# //p'`), `description` = the rest (`tail -n +2`), read only to fill the tool arguments, never retyped.
