...
| `.claude/scripts/pr-feedback.sh` | `fetch` writes a PR's unresolved inline review threads to a file and prints one status line; `reply` posts the implementer's `reply:` lines back on each thread and resolves it — GitHub or Bitbucket, auto-detected. The bridge between the draft PR (where the user reviews) and the workflow. |
